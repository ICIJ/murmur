export default {
  ...
  mounted () {
    window.addEventListener('resize', this.triggerSlide)
  ...
  destroy () {
    window.removeEventListener('resize', this.triggerSlide)
  },
  computed: {
    stylePreTransition () {
      return {
    ...
    activeHeight () {
      return this.active ? (this.scrollHeight || this.$container.scrollHeight) : 0
    },
    $container () {
  ...
  methods: {
    triggerSlide () {
      ...
      this.$nextTick(() => { this.state = 'active' })
    },
    cleanLayout () {
      this.state = 'post'
      this.$nextTick()
